...
Future<String> getAddress() async {
  //Check if address exists? THen read address.
  var val =
...
Future<bool> checkifRefundApproved(
    String productname, String transactionid) async {
  bool returnres = false;
  Query qqq = FirebaseFirestore.instance
      .collection('refunds')
...
Future updateTransactionItems(String transactionid, String itemname) async {
  final CollectionReference productList =
      FirebaseFirestore.instance.collection('transactions');
  return await productList
      .doc(transactionid)
      .update({'itemsandprice.$itemname': FieldValue.delete()});
}

Future<num> getPricePaidinTransaction(String transactionid) async {
  var val = await FirebaseFirestore.instance
      .collection('transactions')
      .doc(transactionid)
      .get();
  num totalprice = val['totalprice'];
  return totalprice;
}

Future updatePricePaidinTransaction(
    String transactionid, num newtotalprice) async {
  final CollectionReference refundList =
      FirebaseFirestore.instance.collection('transactions');
  return await refundList
      .doc(transactionid)
      .update({'totalprice': newtotalprice});
}

Future updateTransactionRefundedItems(
    String transactionid, String itemname, num price) async {
  final CollectionReference productList =
      FirebaseFirestore.instance.collection('transactions');
  return await productList
      .doc(transactionid)
      .update({'refundeditemsandprice.$itemname': price});
}

/*Future<bool> isItemRefunded(String transactionid, String itemname) async {
  bool returnres = false;
  var val = await FirebaseFirestore.instance
      .collection('transactions')
      .doc(transactionid)
      .get();
  Map<String, dynamic> refundedItems = val['refundeditemsandprice'];
  for (var key in refundedItems.keys) {
    if (key == itemname) {
      returnres = true;
    }
  }
