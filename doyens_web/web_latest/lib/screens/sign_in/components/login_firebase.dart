    ...
    return ElevatedButton(
      ...
      onPressed: () async {
        ...
        if (user != null) {
          ...
          if (wishList != null) {
            int iloop = 0;
            for (var v in wishList!.values) {
              if (wishList!.values.elementAt(iloop) > 0) {
                Product toadd = productListnew
                    .where((element) => element.title
                        .contains(wishList!.keys.elementAt(iloop).trimLeft()))
                    .toList()[0];
                currentCart.cartItems!.add(CartItem(
                    product: toadd,
                    numOfItem: wishList!.values.elementAt(iloop)));
              }
              //dbdeki tum itemlari locale cek
              iloop++;
            }
